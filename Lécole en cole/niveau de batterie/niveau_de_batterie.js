...
function initBattery() {
    ...
    navigator.getBattery().then((batt) => {
        updateBattery = () => {
            ...
            if (level == 100) {
                batteryStatus.innerHTML = `Batterie plaine <i class="ri-battery-2-fill green-color"></i>`;
                batteryLiquid.style.height = "103%";
            } else if (level <= 20 & !batt.charging) {
                batteryStatus.innerHTML = `Batterie faible <i class="ri-plug-line animated-red animated-red"></i>`;
            } else if (batt.charging) {
                batteryStatus.innerHTML = `Recharge ... <i class="ri-flashlight-line animated-green"></i>`;
            } else {
